  ...
  return (
    <div className="absolute right-8 shadow-lg mt-2 border-2 border-jetBlack/40 rounded-xl text-left p-2 bg-white z-20">
      ...
      <ul className="text-xl mt-2">
        <Link
          href="/settings"
          className="flex gap-2 pl-2 pr-16 py-2 hover:bg-jetBlack/10 rounded-lg"
        >
          <Settings size="24" />
          Settings
        </Link>
        <li className="flex gap-2 pl-2 py-2 hover:bg-jetBlack/10 cursor-pointer rounded-lg text-red-600" onClick={handleLogout}>
          <LogOut size="24" />
          Log Out
        </li>
      </ul>
